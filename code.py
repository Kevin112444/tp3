...
# Sprites for Golem - https://darkpixel-kronovi.itch.io/mecha-golem-free?download
...
################################################################################      
    def appStarted(self):  
        ...
        self.timerDelay = 1
        self.sheets = []
        self.homeScreen = self.loadImage('Assets/Backgrounds/Homescreen.jpg')
...
# changes the state of the two defensive turrets
...
# combat system for giant, projectiles included
    def giantFight(self):
        giant = self.giants[0]
        if giant.state == 'meleeFight':
...
# damage calculation for the different types
...
# combat system
    def fighting(self):
        ...
        for unit in range(len(self.alive)):
            ...
            #ranged - spawn a projectile for enemy unit to take
            elif self.alive[unit].state == 'fight' and self.alive[unit].range > 0:
                if (self.spriteCounters[unit] == len(self.sprites[unit]) // 2) and len(self.enemyAlive) > 0:
            ...
            # ranged - spawn a projectile for enemy base
...
# mechanic for evolving and auto exp gain
    def evolving(self):
        self.expCounter += 1
    ...
    # same thing but for the two giants
    ...
    def drawHealthbars(self,canvas):
        ...
        canvas.create_rectangle(self.allyBase.relPos-25, .56 * self.height, self.allyBase.relPos - 25 + (50 * (self.allyBase.health / self.allyBase.maxHealth)), .57 * self.height, fill = "green")
        canvas.create_rectangle(self.enemyBase.relPos-25, .56 * self.height, self.enemyBase.relPos + 25, .57 * self.height, fill = "white")
        canvas.create_rectangle(self.enemyBase.relPos-25, .56 * self.height, self.enemyBase.relPos - 25 + (50 * (self.enemyBase.health / self.enemyBase.maxHealth)), .57 * self.height, fill = "green")
